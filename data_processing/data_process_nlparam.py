...
def process_data(input_data_file_path, pct_cutoff, train_size, val_size, test_size, output_file_path):
    ...
    with open(input_data_file_path, 'r', encoding='unicode_escape') as file:
        ...
        for row in reader:
            # Convert raw_score to float and include only rows with valid raw_score
            try:
                ...
                # Skip rows with invalid 'raw_score' or missing keys
                continue
    ...
    for ifpid, group in grouped_by_question:
        ...
        # smaller raw_score = better
        # get the top x% of grouped_by_question_and_date sorted by raw_score and set its label to 0 and the bottom x% label to 1

            # Sort the subgroup by 'raw_score'
            subgroup = subgroup.sort_values(by='raw_score', ascending=True)
            ...
            # Set labels for the top x% (smallest raw_score)
            if top_x_count > 0:
                subgroup.iloc[:top_x_count, subgroup.columns.get_loc('classification_label')] = 1
            
            # Set labels for the bottom x% (largest norm_score)
            if bottom_x_count > 0:
    ...
    # print(data[["ifpid", "date_posted", "raw_score", "classification_label"]].sort_values(by=['ifpid', 'date_posted', 'raw_score']).head(50))
